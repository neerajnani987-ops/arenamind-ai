export function getInterpolatedAnswers(language, telemetry) {
  ...
  if (language === 'telugu') {
    return {
      ...
      washroom: `సమీపంలోని వాష్‌రూమ్: ${restroomN.name || 'Restroom North'} (టైర్ 1).\n- **కారణం**: ${restroomE.name || 'Restroom East'} (టైర్ 2) శుభ్రత కోసం మూసివేయబడింది (సమయం: ${restroomE.waitTime} నిమిషాలు).\n- **విశ్వాసం**: 94%\n- **సిఫార్సు చేసిన చర్య**: నార్త్ కాన్‌కోర్స్ వాష్‌రూమ్‌ని ఉపయోగించండి (${restroomN.waitTime} నిమిషాల నిరీక్షణ).\n- **అంచనా వేసిన ప్రభావం**: వేగంగా వాష్‌రూమ్ సదుపాయాన్ని అందిస్తుంది.`,
  ...
  if (language === 'hindi') {
    return {
      ...
      washroom: `निकटतम शौचालय: ${restroomN.name || 'Restroom North'} (टियर 1)।\n- **कारण**: ${restroomE.name || 'Restroom East'} (टियर 2) निर्धारित सफाई के लिए बंद है (${restroomE.waitTime} मिनट का समय)।\n- **विश्वास**: 94%\n- **अनुशंसित कार्रवाई**: उत्तर कॉन्कोर्स शौचालय पर जाएं (${restroomN.waitTime} मिनट का इंतजार)।\n- **अपेक्षित प्रभाव**: सफाई की कमी को दूर करता है।`,
  ...
  return {
    ...
    food: `Fastest option is ${cafeExpress.name || 'Arena Express Cafe'} (East) with a ${cafeExpress.waitTime}-minute wait.\n- **Reasoning**: ${foodBazaar.name || 'Grand Food Bazaar'} (West) is experiencing a peak order volume (${foodBazaar.waitTime}-minute wait).\n- **Confidence**: 88%\n- **Recommended Action**: Go to the East concourse for quick concessions at ${cafeExpress.name || 'Arena Express Cafe'}.\n- **Expected Impact**: Saves ${Math.max(1, foodBazaar.waitTime - cafeExpress.waitTime)} minutes in line, distributing food service demand evenly across vendors.`,
    washroom: `Nearest restroom: ${restroomN.name || 'Restroom North'} (Tier 1).\n- **Reasoning**: ${restroomE.name || 'Restroom East'} (Tier 2) is closed for scheduled cleaning (wait: ${restroomE.waitTime}m).\n- **Confidence**: 94%\n- **Recommended Action**: Proceed to the North concourse washroom (${restroomN.waitTime}m wait).\n- **Expected Impact**: Restores sanitation access quickly without waiting at congested queues.`,
